...
# =====================================================================================
# XY-router mesh, 4x4: NON-BLOCKING reads, BLOCKING writes (try_get in / put out).
#
# Counterpart of router_XY.py (blocking, always-fire/bubble). Same routing, same
# arbitration -- but the *synchronisation discipline* is gone, because a non-blocking
# poll already answers "is there a flit for me?".
#
# WHY THE ASYMMETRY (this is the design point):
#   INPUT  must be non-blocking. A receiver does not know whether a flit is coming, so
#          it has to poll; a blocking get on an idle link would stall the router.
#   OUTPUT should be BLOCKING. The sender already holds the flit and only needs the
#          channel.  A blocking put IS backpressure: if the downstream FIFO is full the
#          router waits instead of dropping, so the mesh is LOSSLESS with no credit
#          machinery at all -- the six reverse credit-stream arrays, the per-input
#          credit counters and the prime/return protocol of router_XY_PEs_bp.py all
#          collapse into the primitive.  (It also sidesteps the MemRefDCE unused-`ok`
#          deletion entirely, since a blocking put returns no result.)
#
# DEADLOCK: safe by XY/DOR routing -- routing X before Y makes the channel-dependency
# graph acyclic, which is the standard reason XY is used with blocking flow control.
# Two residual risks, both real: (a) a router blocked on one output cannot drain its
# other inputs (head-of-line coupling across ports), and (b) a blocking put to a
# consumer that has already retired hangs instead of dropping -- so the collector must
# outlive the traffic.  Perimeter outputs are never written under correct XY routing to
# an in-mesh destination, but a misrouted flit would hang rather than vanish.
#
...
# NO valid bit -- validity on a read is the try_get `ok` flag (see header).
DW = 16                  # payload width
...
def router_XY_NoC(inj: int32[ROWS, COLUMNS, LANELEN], dlv: int32[ROWS, COLUMNS, LANELEN]):
    ...
    def router_XY():
        ...
        for t in range(NUM_IT):
            ...
            # ── (B) output: BLOCKING put on the granted ports -- idle ports stay silent.
            # Lossless: a full downstream link stalls this router until space frees up,
            # rather than dropping the flit.  No `ok` to consume, so the unused-result
            # deletion that bites try_put cannot apply here.
            if out_vld[0] == 1:
                north[row_id, col_id].put(outpkt[0])
            if out_vld[1] == 1:
                east[row_id, col_id + 1].put(outpkt[1])
            if out_vld[2] == 1:
                south[row_id + 1, col_id].put(outpkt[2])
            if out_vld[3] == 1:
                west[row_id, col_id].put(outpkt[3])
            if out_vld[4] == 1:
                deliver[row_id, col_id].put(outpkt[4])
    ...
    # Blocking put: the driver stalls on a full link rather than dropping a seed, so the
    # pointer simply advances every pass. ──
                    ...
                    with allo.meta_if((r != PE_R) or (c != PE_C)):
                        if sp[r, c] < LANELEN:
                            ...
                            if w != 0:                   # 0 = "no packet in this lane"
                                inject[r, c].put(w)      # blocking: never loses a seed
                            sp[r, c] += 1
    ...
    def pe_mul():
        ...
        for t in range(NUM_IT):
            if txv == 1:                                  # blocking inject of the result
                inject[PE_R, PE_C].put(txp)
                txv = 0
