...
import glob as _glob                                # any v3* variant folder
...
# CHIP selects the chip module: eva_sb_syscredit_rtprime (v2, default),
# eva_v3_channel, eva_v3_nb. Each variant ships its own eva_workloads copy.
...
PARTITION = os.environ.get("PARTITION", "1") == "1"   # SCHED=1 only; see _top()
TS = "_ts" in CHIP or os.environ.get("TS", "0") == "1"   # chip exposes out_cyc_* ports
...
# PRJ is rm -rf'd for non-codegen modes -- override it if another run is using the
# default `generated/` (e.g. a long 8x8 csyn).
...
def _top():
    ...
    if SCHED:
        # PARTITION=0 drops partition_rf: it issues M*N*25 s.partition() calls
        # (1600 at 8x8) but emits NO hls_unroll pragma on this backend, so it may
        # be pure emit cost here. pipeline_node is what produces the II pragma.
        return e.get_scheduled_eva(float16, pipeline_node=True,
                                   partition_rf=PARTITION)
...
# PRIME overrides the runtime prime-token count per tile (rtprime takes it as an
# INPUT, so no rebuild needed). fft/cordic want 1, mmm wants 6 (see the Vitis
# golden_testbenches README: priming is drain-direction-dependent).
_PRIME = int(os.environ.get("PRIME", str(e.PRIME_TOKENS)))
prime_cfg = np.full((M, N), _PRIME, np.int32)           # runtime prime per tile
...
# TS chips add 4 out_cyc_* ports. Discovery order comes from each kernel's
# args=[...] list, and col_X declares args=[out_X, out_cyc_X, ...] -- so each
# timestamp array is discovered immediately after its data array (25 args, not 21).
if TS:
    oc_w, oc_e = zi(M, L), zi(M, L)
    oc_n, oc_s = zi(N, L), zi(N, L)
    mod(prime_cfg,
        in_w, iv_w, in_e, iv_e, in_n, iv_n, in_s, iv_s,
        out_w, oc_w, out_e, oc_e, out_n, oc_n, out_s, oc_s,
...
if TS:
    st = oc_e[0, :K]
    span = int(st[-1] - st[0]) if K > 1 else 0
    print("TIMESTAMPS out_cyc_e:", st.tolist(),
          f"-> span {span} cyc over {K} values = {span/(K-1):.2f} cyc/val" if K > 1 else "")
