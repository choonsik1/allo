...
TS = "_ts" in CHIP or os.environ.get("TS", "0") == "1"   # chip has out_cyc_* ports
# PRJ is rm -rf'd below, so allow an override to avoid clobbering a concurrent run
# using the default `generated/` (e.g. a long 8x8 csyn).
PRJ = os.environ.get("PRJ", os.path.join(ROOT, "generated"))

# ---- workload: 1x1 MMM, B batches. W is (M,N)=(1,1), X is (B,M)=(B,1). ----
# MMM_MESH sets the array size: W is (M,N)=(mesh,mesh), X is (B,M)=(B,mesh).
# load_mmm_router takes M,N from these shapes and uses off=0 (NO static skew) --
# timing is meant to come from data-driven stalling / the link handshake.
# Values kept small so every fp16 dot product is exact (no rounding in the golden).
...
if _MESH == 1:
    ...
    W = np.array([[float(1 + ((i + j) % 3)) for j in range(_MESH)]
                  for i in range(_MESH)], np.float16)
    X = np.array([[float(1 + ((b + i) % 3)) for i in range(_MESH)]
                  for b in range(_B)], np.float16)
...
# SCHED=1 applies the Allo schedule -> `#pragma HLS pipeline II=1` on each node's t loop
# (get_scheduled_eva -> s.pipeline("node_i_j:t", initiation_interval=1)). Worth re-testing:
# the old "II pragmas are strictly worse" verdict was measured in LOOP ITERATIONS, which
# is blind to clocks-per-iteration -- exactly what a pipeline pragma changes.
SCHED = os.environ.get("SCHED", "0") == "1"
if SCHED:
    _s = e.get_scheduled_eva(float16, pipeline_node=True,
                             partition_rf=os.environ.get("PARTITION", "0") == "1")
...
print(f"emitted -> {PRJ}  (SCHED={int(SCHED)})", flush=True)

_PRIME = int(os.environ.get("PRIME", str(e.PRIME_TOKENS)))
prime_cfg = np.full((e.M, e.N), _PRIME, np.int32)
...
if TS:
    _z = lambda a: np.zeros(a.shape, np.int32)
    ocs = [_z(a) for a in args["outs"]]
    o = args["outs"]
    mod(prime_cfg,
        ins[0], ivs[0], ins[1], ivs[1], ins[2], ivs[2], ins[3], ivs[3],
        o[0], ocs[0], o[1], ocs[1], o[2], ocs[2], o[3], ocs[3],
        *args["rins"], *args["routs"])
else:
    mod(prime_cfg,
        ins[0], ivs[0], ins[1], ivs[1], ins[2], ivs[2], ins[3], ivs[3],
