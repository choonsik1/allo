...
FP_LAT  = 1 if _CHIP_IS_FLOAT else 0
MOV_LAT = 1 if _CHIP_IS_FLOAT else 0
DATADRIVEN = 1
            ...
            for it in range(BUDGET):
                ...
                if txp_v[0] == 1 and sys_n[i, j].full() == 0:
                    with allo.meta_if(IS_FLOAT):
                        sys_n[i, j].put(txp_d[0].bitcast())
                    with allo.meta_else():
                        sys_n[i, j].put(txp_d[0] & ((1 << DATA_W) - 1))
                    txp_v[0] = 0
                if txp_v[1] == 1 and sys_s[i + 1, j].full() == 0:
                    with allo.meta_if(IS_FLOAT):
                        sys_s[i + 1, j].put(txp_d[1].bitcast())
                    with allo.meta_else():
                        sys_s[i + 1, j].put(txp_d[1] & ((1 << DATA_W) - 1))
                    txp_v[1] = 0
                if txp_v[2] == 1 and sys_w[i, j].full() == 0:
                    with allo.meta_if(IS_FLOAT):
                        sys_w[i, j].put(txp_d[2].bitcast())
                    with allo.meta_else():
                        sys_w[i, j].put(txp_d[2] & ((1 << DATA_W) - 1))
                    txp_v[2] = 0
                if txp_v[3] == 1 and sys_e[i, j + 1].full() == 0:
                    with allo.meta_if(IS_FLOAT):
                        sys_e[i, j + 1].put(txp_d[3].bitcast())
                    with allo.meta_else():
                        sys_e[i, j + 1].put(txp_d[3] & ((1 << DATA_W) - 1))
                    txp_v[3] = 0
                ...
                # (stripped) register dests no longer retire to drf -> no backpressure gate
                if sb_v[0] == 1 and retire_ok == 1:
                    ...
                    if sb_rtr[0] == 1:
                        ...
                        pass  # (stripped) register writeback happens immediately at issue
                ...
                # SCOREBOARD STRIPPED (int16 experiment): no resq operand-forwarding and
                # no res->resq RAW stall.  Operands come straight from drf; the drf_full
                # valid bit + immediate write-back (below) guarantee freshness.  Only the
                # compare interlock (cmp_busy) is kept -- the condition path still retires
                # through the sb pipeline.  raw stays 0 (declared above).
                ...
                # IMMEDIATE WRITE-BACK (scoreboard stripped): register results go straight
                # to drf the cycle they are produced, so the next dependent op reads a fresh
                # value from drf (single-cycle int16) without any resq forwarding.
                if grant == 1 and dst < 12:
                ...
                with allo.meta_for(0, M) as r:
                    if sp[r] < LANELEN:
                        if vd_w[r, sp[r]] == 0:
                            ...
                            with allo.meta_else():
                                sys_e[r, 0].put(din_w[r, sp[r]] & ((1 << DATA_W) - 1))
                            sp[r] += 1
                ...
                with allo.meta_for(0, M) as r:
                    if sp[r] < LANELEN:
                        if vd_e[r, sp[r]] == 0:
                            ...
                            with allo.meta_else():
                                sys_w[r, N].put(din_e[r, sp[r]] & ((1 << DATA_W) - 1))
                            sp[r] += 1
                ...
                with allo.meta_for(0, N) as c:
                    if sp[c] < LANELEN:
                        if vd_n[c, sp[c]] == 0:
                            ...
                            with allo.meta_else():
                                sys_s[0, c].put(din_n[c, sp[c]] & ((1 << DATA_W) - 1))
                            sp[c] += 1
                ...
                with allo.meta_for(0, N) as c:
                    if sp[c] < LANELEN:
                        if vd_s[c, sp[c]] == 0:
                            ...
                            with allo.meta_else():
                                sys_n[M, c].put(din_s[c, sp[c]] & ((1 << DATA_W) - 1))
                            sp[c] += 1
