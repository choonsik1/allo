...
def get_eva_top_elastic(Ty: AlloType = float16):
    ...
    import os as _os
    _BM = float(_os.environ.get('BUDMUL', '6'))   # 6x is the chip's own floor.
    # Is it NECESSARY or merely conservative? Break-even vs v16 at 4x4 is 4.04x.
    # Correctness is the arbiter: bit-exact, or the cycle number is void.
    BUDGET = RUN_BUDGET if RUN_BUDGET >= _BM * LANELEN else int(_BM * LANELEN)
