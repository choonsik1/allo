...
FWD = 1
TY = int16                                  # THE one-token datatype selector (fp16 file: TY = float16)
_CHIP_IS_FLOAT = TY in (float16, float32)   # everything below derives from TY — no drift
# EVA_ALU_LAT: forwarding-readiness threshold for the scoreboard bypass (lines ~318).
# need=0 lets an instruction forward from the NEWEST scoreboard entry -- i.e. the ALU
# result reaches the operand mux combinationally in the same cycle, which is exactly the
# int16 critical path (acc -> drf_full/scoreboard). need=1 blocks that bypass, forcing a
# one-cycle RAW stall and cutting the loop. fp16 has always run need=1 (FP_LAT=MOV_LAT=1);
# this knob lets int16 buy the same register at a throughput cost, for the freq comparison.
# Unset -> the historical per-datatype default, so existing runs do not drift.
import os as _os
_ALU_LAT = _os.environ.get("EVA_ALU_LAT")
_DEF_LAT = 1 if _CHIP_IS_FLOAT else 0
FP_LAT  = int(_ALU_LAT) if _ALU_LAT is not None else _DEF_LAT
MOV_LAT = int(_ALU_LAT) if _ALU_LAT is not None else _DEF_LAT
DATADRIVEN = 1
...
def get_eva_top_elastic(Ty: AlloType = float16):
    ...
    # The 6*LANELEN floor guards against silently under-budgeting a real run.
    # EVA_BUDGET_FLOOR=0 lifts it so RUN_BUDGET can be swept DOWN to find the
    # smallest step count that still collects every output -- that minimum is the
    # program's true cycle cost, which is how the EVA_ALU_LAT throughput penalty
    # gets measured (csim runs a fixed trip count, so nothing else reveals it).
    _FLOOR = int(_os.environ.get('EVA_BUDGET_FLOOR', '1'))
    BUDGET = RUN_BUDGET if (RUN_BUDGET >= 6 * LANELEN or not _FLOOR) else 6 * LANELEN
    ...
    def top(
        ...
        def node():
            ...
            for it in range(BUDGET):
                if oh_v[0] == 1:
                    _ok1 = rtr_e[i, j + 1].try_put(oh_p[0])
                    if _ok1 == 1:
                        oh_v[0] = 0
                if oh_v[1] == 1:
                    _ok2 = rtr_w[i, j].try_put(oh_p[1])
                    if _ok2 == 1:
                        oh_v[1] = 0
                if oh_v[2] == 1:
                    _ok3 = rtr_s[i + 1, j].try_put(oh_p[2])
                    if _ok3 == 1:
                        oh_v[2] = 0
                if oh_v[3] == 1:
                    _ok4 = rtr_n[i, j].try_put(oh_p[3])
                    if _ok4 == 1:
                        oh_v[3] = 0
                if txp_v[0] == 1:
                    _v5: SDATA = 0
                    with allo.meta_if(IS_FLOAT):
                        _v5 = txp_d[0].bitcast()
                    with allo.meta_else():
                        _v5 = txp_d[0] & ((1 << DATA_W) - 1)
                    _ok5 = sys_n[i, j].try_put(_v5)
                    if _ok5 == 1:
                        txp_v[0] = 0
                if txp_v[1] == 1:
                    _v6: SDATA = 0
                    with allo.meta_if(IS_FLOAT):
                        _v6 = txp_d[1].bitcast()
                    with allo.meta_else():
                        _v6 = txp_d[1] & ((1 << DATA_W) - 1)
                    _ok6 = sys_s[i + 1, j].try_put(_v6)
                    if _ok6 == 1:
                        txp_v[1] = 0
                if txp_v[2] == 1:
                    _v7: SDATA = 0
                    with allo.meta_if(IS_FLOAT):
                        _v7 = txp_d[2].bitcast()
                    with allo.meta_else():
                        _v7 = txp_d[2] & ((1 << DATA_W) - 1)
                    _ok7 = sys_w[i, j].try_put(_v7)
                    if _ok7 == 1:
                        txp_v[2] = 0
                if txp_v[3] == 1:
                    _v8: SDATA = 0
                    with allo.meta_if(IS_FLOAT):
                        _v8 = txp_d[3].bitcast()
                    with allo.meta_else():
                        _v8 = txp_d[3] & ((1 << DATA_W) - 1)
                    _ok8 = sys_e[i, j + 1].try_put(_v8)
                    if _ok8 == 1:
                        txp_v[3] = 0
                if ig_v[0] == 0:
                    _g17, _k17 = rtr_e[i, j].try_get()
                    if _k17 == 1: ig_p[0] = _g17; ig_v[0] = 1
                if ig_v[1] == 0:
                    _g18, _k18 = rtr_w[i, j + 1].try_get()
                    if _k18 == 1: ig_p[1] = _g18; ig_v[1] = 1
                if ig_v[2] == 0:
                    _g19, _k19 = rtr_s[i, j].try_get()
                    if _k19 == 1: ig_p[2] = _g19; ig_v[2] = 1
                if ig_v[3] == 0:
                    _g20, _k20 = rtr_n[i + 1, j].try_get()
                    if _k20 == 1: ig_p[3] = _g20; ig_v[3] = 1

                if hold_cnt[0] < 2:
                    _g21, _k21 = sys_s[i, j].try_get()
                    if _k21 == 1:
                        wN: SDATA = _g21
                        with allo.meta_if(IS_FLOAT):
                            hold_v[0, hold_cnt[0]] = wN.bitcast()
                        with allo.meta_else():
                            hold_v[0, hold_cnt[0]] = wN & ((1 << DATA_W) - 1)
                        hold_cnt[0] += 1
                if hold_cnt[1] < 2:
                    _g22, _k22 = sys_n[i + 1, j].try_get()
                    if _k22 == 1:
                        wS: SDATA = _g22
                        with allo.meta_if(IS_FLOAT):
                            hold_v[1, hold_cnt[1]] = wS.bitcast()
                        with allo.meta_else():
                            hold_v[1, hold_cnt[1]] = wS & ((1 << DATA_W) - 1)
                        hold_cnt[1] += 1
                if hold_cnt[2] < 2:
                    _g23, _k23 = sys_e[i, j].try_get()
                    if _k23 == 1:
                        wW: SDATA = _g23
                        with allo.meta_if(IS_FLOAT):
                            hold_v[2, hold_cnt[2]] = wW.bitcast()
                        with allo.meta_else():
                            hold_v[2, hold_cnt[2]] = wW & ((1 << DATA_W) - 1)
                        hold_cnt[2] += 1
                if hold_cnt[3] < 2:
                    _g24, _k24 = sys_w[i, j + 1].try_get()
                    if _k24 == 1:
                        wE: SDATA = _g24
                        with allo.meta_if(IS_FLOAT):
                            hold_v[3, hold_cnt[3]] = wE.bitcast()
                        with allo.meta_else():
                            hold_v[3, hold_cnt[3]] = wE & ((1 << DATA_W) - 1)
                        hold_cnt[3] += 1
                ...
                with allo.meta_for(0, M) as r:
                    if sp[r] < LANELEN:
                        if vd_w[r, sp[r]] == 0:
                            ...
                            _v9: SDATA = 0
                            with allo.meta_if(IS_FLOAT):
                                _v9 = din_w[r, sp[r]].bitcast()
                            with allo.meta_else():
                                _v9 = din_w[r, sp[r]] & ((1 << DATA_W) - 1)
                            _ok9 = sys_e[r, 0].try_put(_v9)
                            if _ok9 == 1:
                                sp[r] += 1
                ...
                with allo.meta_for(0, M) as r:
                    if sp[r] < LANELEN:
                        if vd_e[r, sp[r]] == 0:
                            ...
                            _v10: SDATA = 0
                            with allo.meta_if(IS_FLOAT):
                                _v10 = din_e[r, sp[r]].bitcast()
                            with allo.meta_else():
                                _v10 = din_e[r, sp[r]] & ((1 << DATA_W) - 1)
                            _ok10 = sys_w[r, N].try_put(_v10)
                            if _ok10 == 1:
                                sp[r] += 1
                ...
                with allo.meta_for(0, N) as c:
                    if sp[c] < LANELEN:
                        if vd_n[c, sp[c]] == 0:
                            ...
                            _v11: SDATA = 0
                            with allo.meta_if(IS_FLOAT):
                                _v11 = din_n[c, sp[c]].bitcast()
                            with allo.meta_else():
                                _v11 = din_n[c, sp[c]] & ((1 << DATA_W) - 1)
                            _ok11 = sys_s[0, c].try_put(_v11)
                            if _ok11 == 1:
                                sp[c] += 1
                ...
                with allo.meta_for(0, N) as c:
                    if sp[c] < LANELEN:
                        if vd_s[c, sp[c]] == 0:
                            ...
                            _v12: SDATA = 0
                            with allo.meta_if(IS_FLOAT):
                                _v12 = din_s[c, sp[c]].bitcast()
                            with allo.meta_else():
                                _v12 = din_s[c, sp[c]] & ((1 << DATA_W) - 1)
                            _ok12 = sys_n[M, c].try_put(_v12)
                            if _ok12 == 1:
                                sp[c] += 1
...
if __name__ == "__main__":
    ...
    s = df.customize(get_eva_top_elastic(TY))
    for i in range(M):
        for j in range(N):
    ...
    if _CHIP_IS_FLOAT:   # fp16 needs the distance-2 directive to reach II=1; int16 closes at II=1 unaided (NODEP)
