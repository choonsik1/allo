...
FWD = 1
# EVA_ALU_LAT: forwarding-readiness threshold for the scoreboard bypass (see ~L270).
# need=0 lets an instruction forward from the NEWEST scoreboard entry, i.e. the ALU
# result reaches the operand mux combinationally in the same cycle. need=1 blocks that
# bypass (one-cycle RAW stall instead) and cuts the loop-carried recurrence on resq.
# fp16 default is 1; EVA_ALU_LAT=0 is the CONTROL for the int16-vs-fp16 frequency gap.
# NOTE: the Vitis __main__ path asserts MOV_LAT>=1 before injecting the distance-2
# dependence directive -- EVA_ALU_LAT=0 is valid for the Catapult csyn path only.
import os as _os
_ALU_LAT = _os.environ.get("EVA_ALU_LAT")
FP_LAT  = int(_ALU_LAT) if _ALU_LAT is not None else 1
MOV_LAT = int(_ALU_LAT) if _ALU_LAT is not None else 1
DATADRIVEN = 1
    ...
    def top(
        ...
        def node():
            ...
            for it in range(BUDGET):
                ...
                if txp_v[0] == 1:
                    _ok5 = sys_n[i, j].try_put(txp_d[0].bitcast())
                    if _ok5 == 1: txp_v[0] = 0
                if txp_v[1] == 1:
                    _ok6 = sys_s[i + 1, j].try_put(txp_d[1].bitcast())
                    if _ok6 == 1: txp_v[1] = 0
                if txp_v[2] == 1:
                    _ok7 = sys_w[i, j].try_put(txp_d[2].bitcast())
                    if _ok7 == 1: txp_v[2] = 0
                if txp_v[3] == 1:
                    _ok8 = sys_e[i, j + 1].try_put(txp_d[3].bitcast())
                    if _ok8 == 1: txp_v[3] = 0

                if ig_v[0] == 0:
                    _g9, _k9 = rtr_e[i, j].try_get()
                    if _k9 == 1: ig_p[0] = _g9; ig_v[0] = 1
                if ig_v[1] == 0:
                    _g10, _k10 = rtr_w[i, j + 1].try_get()
                    if _k10 == 1: ig_p[1] = _g10; ig_v[1] = 1
                if ig_v[2] == 0:
                    _g11, _k11 = rtr_s[i, j].try_get()
                    if _k11 == 1: ig_p[2] = _g11; ig_v[2] = 1
                if ig_v[3] == 0:
                    _g12, _k12 = rtr_n[i + 1, j].try_get()
                    if _k12 == 1: ig_p[3] = _g12; ig_v[3] = 1

                if hold_cnt[0] < 2:
                    _g13, _k13 = sys_s[i, j].try_get()
                    if _k13 == 1:
                        wN: SDATA = _g13
                        hold_v[0, hold_cnt[0]] = wN.bitcast(); hold_cnt[0] += 1
                if hold_cnt[1] < 2:
                    _g14, _k14 = sys_n[i + 1, j].try_get()
                    if _k14 == 1:
                        wS: SDATA = _g14
                        hold_v[1, hold_cnt[1]] = wS.bitcast(); hold_cnt[1] += 1
                if hold_cnt[2] < 2:
                    _g15, _k15 = sys_e[i, j].try_get()
                    if _k15 == 1:
                        wW: SDATA = _g15
                        hold_v[2, hold_cnt[2]] = wW.bitcast(); hold_cnt[2] += 1
                if hold_cnt[3] < 2:
                    _g16, _k16 = sys_w[i, j + 1].try_get()
                    if _k16 == 1:
                        wE: SDATA = _g16
                ...
                with allo.meta_for(0, M) as r:
                    if sp[r] < LANELEN:
                        if vd_w[r, sp[r]] == 0:
                            ...
                            _ok101 = sys_e[r, 0].try_put(din_w[r, sp[r]].bitcast())
                            if _ok101 == 1:
                                sp[r] += 1
                ...
                with allo.meta_for(0, M) as r:
                    if sp[r] < LANELEN:
                        if vd_e[r, sp[r]] == 0:
                            ...
                            _ok102 = sys_w[r, N].try_put(din_e[r, sp[r]].bitcast())
                            if _ok102 == 1:
                                sp[r] += 1
                ...
                with allo.meta_for(0, N) as c:
                    if sp[c] < LANELEN:
                        if vd_n[c, sp[c]] == 0:
                            ...
                            _ok103 = sys_s[0, c].try_put(din_n[c, sp[c]].bitcast())
                            if _ok103 == 1:
                                sp[c] += 1
                ...
                with allo.meta_for(0, N) as c:
                    if sp[c] < LANELEN:
                        if vd_s[c, sp[c]] == 0:
                            ...
                            _ok104 = sys_n[M, c].try_put(din_s[c, sp[c]].bitcast())
                            if _ok104 == 1:
                                sp[c] += 1
