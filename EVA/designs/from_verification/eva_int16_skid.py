...
FP_LAT  = int(_ALU_LAT) if _ALU_LAT is not None else 0   # int16 default: single-cycle datapath
MOV_LAT = int(_ALU_LAT) if _ALU_LAT is not None else 0
...
def get_eva_top(Ty: AlloType = float16):
    DATA_W = Ty.bits
    IS_FLOAT = "float" in str(Ty).lower()
    ...
    def top(
        ...
        def node(pcfg: int32[M, N]):
            ...
            for t in range(NSTEP):
                ...
                with allo.meta_if(IS_FLOAT): crv_data = o_crv[0 : Ty.bits].bitcast()
                with allo.meta_else(): crv_data = o_crv[0 : Ty.bits]
                ...
                with allo.meta_if(IS_FLOAT): rxv[0] = rx_n[1 : 1 + Ty.bits].bitcast()
                with allo.meta_else(): rxv[0] = rx_n[1 : 1 + Ty.bits]
                rxvld[0] = rx_n[0]
                with allo.meta_if(IS_FLOAT): rxv[1] = rx_s[1 : 1 + Ty.bits].bitcast()
                with allo.meta_else(): rxv[1] = rx_s[1 : 1 + Ty.bits]
                rxvld[1] = rx_s[0]
                with allo.meta_if(IS_FLOAT): rxv[2] = rx_w[1 : 1 + Ty.bits].bitcast()
                with allo.meta_else(): rxv[2] = rx_w[1 : 1 + Ty.bits]
                rxvld[2] = rx_w[0]
                with allo.meta_if(IS_FLOAT): rxv[3] = rx_e[1 : 1 + Ty.bits].bitcast()
                with allo.meta_else(): rxv[3] = rx_e[1 : 1 + Ty.bits]
                rxvld[3] = rx_e[0]
                for d in range(4):
                ...
                if sb_v[0] == 1 and retire_ok == 1:
                    ...
                    if sb_rtr[0] == 1:
                        if sb_inj[0] == 1 and csd_pkt[RQ_OFF] == 0:
                            with allo.meta_if(IS_FLOAT): csd_pkt[0 : Ty.bits] = wb.bitcast()
                            with allo.meta_else(): csd_pkt[0 : Ty.bits] = wb
                ...
                if txp_v[0] == 1 and scred[0] > 0:
                    twn: SYS_W = 0
                    twn[0] = 1
                    with allo.meta_if(IS_FLOAT): twn[1 : 1 + Ty.bits] = txp_d[0].bitcast()
                    with allo.meta_else(): twn[1 : 1 + Ty.bits] = txp_d[0]
                    txn_r = twn; txp_v[0] = 0; scred[0] -= 1
                if txp_v[1] == 1 and scred[1] > 0:
                    tws: SYS_W = 0
                    tws[0] = 1
                    with allo.meta_if(IS_FLOAT): tws[1 : 1 + Ty.bits] = txp_d[1].bitcast()
                    with allo.meta_else(): tws[1 : 1 + Ty.bits] = txp_d[1]
                    txs_r = tws; txp_v[1] = 0; scred[1] -= 1
                if txp_v[2] == 1 and scred[2] > 0:
                    tww: SYS_W = 0
                    tww[0] = 1
                    with allo.meta_if(IS_FLOAT): tww[1 : 1 + Ty.bits] = txp_d[2].bitcast()
                    with allo.meta_else(): tww[1 : 1 + Ty.bits] = txp_d[2]
                    txw_r = tww; txp_v[2] = 0; scred[2] -= 1
                if txp_v[3] == 1 and scred[3] > 0:
                    twe: SYS_W = 0
                    twe[0] = 1
                    with allo.meta_if(IS_FLOAT): twe[1 : 1 + Ty.bits] = txp_d[3].bitcast()
                    with allo.meta_else(): twe[1 : 1 + Ty.bits] = txp_d[3]
                ...
                with allo.meta_for(0, M) as r:
                    ...
                    if wcnt[r] > 0 and t >= win_s[r, 0]:
                        if win_f[r, 0] == 0:
                            ...
                            w[0] = 1
                            with allo.meta_if(IS_FLOAT): w[1 : 1 + Ty.bits] = win_d[r, 0].bitcast()
                            with allo.meta_else(): w[1 : 1 + Ty.bits] = win_d[r, 0]
                            dcred[r] -= 1
                            popd = 1
                    if popd == 1:
            ...
            for t in range(NSTEP):
                with allo.meta_for(0, M) as r:
                    ...
                    if w[0] == 1:
                        cret[r] = 1
                        if k[r] < LANELEN:
                            with allo.meta_if(IS_FLOAT): dout_w[r, k[r]] = w[1 : 1 + Ty.bits].bitcast()
                            with allo.meta_else(): dout_w[r, k[r]] = w[1 : 1 + Ty.bits]
                            k[r] += 1
                    scr_w[r, 0].put(cret[r])
            ...
            for t in range(NSTEP):
                with allo.meta_for(0, M) as r:
                    ...
                    if w[0] == 1:
                        cret[r] = 1
                        if k[r] < LANELEN:
                            with allo.meta_if(IS_FLOAT): dout_e[r, k[r]] = w[1 : 1 + Ty.bits].bitcast()
                            with allo.meta_else(): dout_e[r, k[r]] = w[1 : 1 + Ty.bits]
                            k[r] += 1
                    scr_e[r, N].put(cret[r])
            ...
            for t in range(NSTEP):
                with allo.meta_for(0, N) as c:
                    ...
                    if w[0] == 1:
                        cret[c] = 1
                        if k[c] < LANELEN:
                            with allo.meta_if(IS_FLOAT): dout_n[c, k[c]] = w[1 : 1 + Ty.bits].bitcast()
                            with allo.meta_else(): dout_n[c, k[c]] = w[1 : 1 + Ty.bits]
                            k[c] += 1
                    scr_n[0, c].put(cret[c])
            ...
            for t in range(NSTEP):
                with allo.meta_for(0, N) as c:
                    ...
                    if w[0] == 1:
                        cret[c] = 1
                        if k[c] < LANELEN:
                            with allo.meta_if(IS_FLOAT): dout_s[c, k[c]] = w[1 : 1 + Ty.bits].bitcast()
                            with allo.meta_else(): dout_s[c, k[c]] = w[1 : 1 + Ty.bits]
                            k[c] += 1
                    scr_s[M, c].put(cret[c])
...
if __name__ == "__main__":
    ...
    s = df.customize(get_eva_top(int16))
    for i in range(M):
        for j in range(N):
