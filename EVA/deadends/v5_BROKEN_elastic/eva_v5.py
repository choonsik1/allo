...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        # TIMESTAMP outputs (cosim): collectors stamp the production cycle t
        out_cyc_w: int32[M, LANELEN], out_cyc_e: int32[M, LANELEN],
        out_cyc_n: int32[N, LANELEN], out_cyc_s: int32[N, LANELEN],
        ...
        def node(pcfg: int32[M, N]):
            ...
            for t in range(NSTEP):
                ...
                for o in range(4):
                    # v3.2: router credit guard REMOVED (Channel valid_ready backpressure)
                    if idir == o:
                ...
                # v3.5 NON-BLOCKING backpressure: try_get() only when this direction's
                # 2-deep hold has room. The old UNCONDITIONAL get() + "if hold_cnt<2"
                # store silently DROPPED words once the hold filled -- the credit plane
                # existed solely to make that drop unreachable. A blocking get() gated
                # on capacity DEADLOCKS (a stalled thread also stops its own put()s), so
                # use try_get: the word stays in the channel and valid_ready holds the
                # sender off. Not receiving leaves rx_* = 0, whose vld bit is already 0,
                # so rxvld needs no change. hold: 0<-rx_n 1<-rx_s 2<-rx_w 3<-rx_e
                rx_w: SYS_W = 0
                rx_e: SYS_W = 0
                rx_n: SYS_W = 0
                rx_s: SYS_W = 0
                if hold_cnt[2] < 2:
                    tw, okw = sys_e[i, j].try_get()
                    if okw == 1: rx_w = tw
                if hold_cnt[3] < 2:
                    te, oke = sys_w[i, j + 1].try_get()
                    if oke == 1: rx_e = te
                if hold_cnt[0] < 2:
                    tn, okn = sys_s[i, j].try_get()
                    if okn == 1: rx_n = tn
                if hold_cnt[1] < 2:
                    ts_, oks = sys_n[i + 1, j].try_get()
                    if oks == 1: rx_s = ts_
                ...
                if txp_v[0] == 1:
                    ...
                    txn_r = twn   # v5: KEEP txp_v[0] set -- cleared only
                                 # when try_put actually lands (rq held until gt)
                if txp_v[1] == 1:
                    tws: SYS_W = 0
                    tws[0] = 1
                    tws[1 : 1 + Ty.bits] = txp_d[1].bitcast()
                    txs_r = tws   # v5: KEEP txp_v[1] set -- cleared only
                                 # when try_put actually lands (rq held until gt)
                if txp_v[2] == 1:
                    tww: SYS_W = 0
                    tww[0] = 1
                    tww[1 : 1 + Ty.bits] = txp_d[2].bitcast()
                    txw_r = tww   # v5: KEEP txp_v[2] set -- cleared only
                                 # when try_put actually lands (rq held until gt)
                if txp_v[3] == 1:
                    twe: SYS_W = 0
                    twe[0] = 1
                    twe[1 : 1 + Ty.bits] = txp_d[3].bitcast()
                    txe_r = twe   # v5: KEEP txp_v[3] set -- cleared only
                                 # when try_put actually lands (rq held until gt)
                if crv_vld == 1:
                    if crv_mode == 1:
                ...
                rtr_s[i + 1, j].put(os_r)
                rtr_n[i, j].put(on_r)
                # v4 NO ALWAYS-FIRE: emit ONLY real words. The always-fire model put a
                # bubble on every link every cycle; paired with the gated try_get receive
                # that backs the links up (v3.5 livelocked). With both sides conditional,
                # a link carries only valid data and valid_ready supplies the backpressure.
                # v5 RQ/GT: try_put == "assert rq"; success == "gt". A word that is
                # not granted stays pending (txp_v still set) and is retried next
                # cycle -- stall-not-drop, exactly what the credit plane emulated and
                # what the golden EVA rq/gt does. A BLOCKING put here would freeze the
                # whole SC_THREAD (that is v4's livelock), which is NOT what the RTL does.
                if txe_r[0] == 1:
                    oke = sys_e[i, j + 1].try_put(txe_r)
                    if oke == 1: txp_v[3] = 0
                if txw_r[0] == 1:
                    okw = sys_w[i, j].try_put(txw_r)
                    if okw == 1: txp_v[2] = 0
                if txs_r[0] == 1:
                    oks = sys_s[i + 1, j].try_put(txs_r)
                    if oks == 1: txp_v[1] = 0
                if txn_r[0] == 1:
                    okn = sys_n[i, j].try_put(txn_r)
                    if okn == 1: txp_v[0] = 0
                ...
                with allo.meta_for(0, M) as r:
                    ...
                    if sp[r] < LANELEN and t >= sp[r]:
                        if vd_w[r, sp[r]] == 0:
                            ...
                            sp[r] += 1
                    if w[0] == 1:
                        okd = sys_e[r, 0].try_put(w)
                        if okd == 0: sp[r] -= 1   # not granted: re-send this slot next cycle
                ...
                with allo.meta_for(0, N) as c:
                    ...
                    if sp[c] < LANELEN and t >= sp[c]:
                        if vd_n[c, sp[c]] == 0:
                            ...
                            sp[c] += 1
                    if w[0] == 1:
                        okd = sys_s[0, c].try_put(w)
                        if okd == 0: sp[c] -= 1   # not granted: re-send this slot next cycle
                ...
                with allo.meta_for(0, N) as c:
                    ...
                    if sp[c] < LANELEN and t >= sp[c]:
                        if vd_s[c, sp[c]] == 0:
                            ...
                            sp[c] += 1
                    if w[0] == 1:
                        okd = sys_n[M, c].try_put(w)
                        if okd == 0: sp[c] -= 1   # not granted: re-send this slot next cycle

        @df.kernel(mapping=[1], args=[out_w, out_cyc_w, prime_cfg])
        def col_w(dout_w: Ty[M, LANELEN], dcyc_w: int32[M, LANELEN], pcfg: int32[M, N]):
        ...
        @df.kernel(mapping=[1], args=[out_e, out_cyc_e, prime_cfg])
        def col_e(dout_e: Ty[M, LANELEN], dcyc_e: int32[M, LANELEN], pcfg: int32[M, N]):
            # SYS-CREDIT collector: consumes every word, returns a credit per
            # real word (rclc pattern; pre-put 2 = advertised hold capacity)
            k: int32[M] = 0
            cret: int32[M] = 0
        ...
        @df.kernel(mapping=[1], args=[out_n, out_cyc_n, prime_cfg])
        def col_n(dout_n: Ty[N, LANELEN], dcyc_n: int32[N, LANELEN], pcfg: int32[M, N]):
            # SYS-CREDIT collector: consumes every word, returns a credit per
            # real word (rclc pattern; pre-put 2 = advertised hold capacity)
            k: int32[N] = 0
            cret: int32[N] = 0
        ...
        @df.kernel(mapping=[1], args=[out_s, out_cyc_s, prime_cfg])
        def col_s(dout_s: Ty[N, LANELEN], dcyc_s: int32[N, LANELEN], pcfg: int32[M, N]):
            # SYS-CREDIT collector: consumes every word, returns a credit per
            # real word (rclc pattern; pre-put 2 = advertised hold capacity)
            k: int32[N] = 0
            cret: int32[N] = 0
...
def get_eva_top_extended(Ty: AlloType = float16):
    ...
    def top_extended(
        ...
        @df.kernel(mapping=[1], args=[out_w, out_cyc_w])
        def col_w(dout_w: Ty[M, LANELEN], dcyc_w: int32[M, LANELEN]):
            # SYS-CREDIT collector: consumes every word, returns a credit per
            # real word (rclc pattern; pre-put 2 = advertised hold capacity)
            k: int32[M] = 0
            cret: int32[M] = 0
        ...
        @df.kernel(mapping=[1], args=[out_e, out_cyc_e])
        def col_e(dout_e: Ty[M, LANELEN], dcyc_e: int32[M, LANELEN]):
            # SYS-CREDIT collector: consumes every word, returns a credit per
            # real word (rclc pattern; pre-put 2 = advertised hold capacity)
            k: int32[M] = 0
            cret: int32[M] = 0
        ...
        @df.kernel(mapping=[1], args=[out_n, out_cyc_n])
        def col_n(dout_n: Ty[N, LANELEN], dcyc_n: int32[N, LANELEN]):
            # SYS-CREDIT collector: consumes every word, returns a credit per
            # real word (rclc pattern; pre-put 2 = advertised hold capacity)
            k: int32[N] = 0
            cret: int32[N] = 0
        ...
        @df.kernel(mapping=[1], args=[out_s, out_cyc_s])
        def col_s(dout_s: Ty[N, LANELEN], dcyc_s: int32[N, LANELEN]):
            # SYS-CREDIT collector: consumes every word, returns a credit per
            # real word (rclc pattern; pre-put 2 = advertised hold capacity)
            k: int32[N] = 0
            cret: int32[N] = 0
