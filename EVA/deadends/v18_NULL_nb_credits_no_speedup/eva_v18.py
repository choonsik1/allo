...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        def node(pcfg: int32[M, N]):
            ...
            # v18: a refused credit try_put must not lose the credit -- accumulate and
            # retry. Credits are additive, so sending the running sum == sending each.
            cq_e: UInt(8) = 0; cq_w: UInt(8) = 0; cq_s: UInt(8) = 0; cq_n: UInt(8) = 0
            ...
            for _pt in range(pcfg[i, j] - 1):
                ...
                _c = cr_e[i, j].try_put(zcr)   # v18: uniform PushNB
                _c = cr_w[i, j + 1].try_put(zcr)
                _c = cr_s[i, j].try_put(zcr)
                _c = cr_n[i + 1, j].try_put(zcr)
            ...
            cq_e += cre_r
            _pe = cr_e[i, j].try_put(cq_e)
            if _pe == 1: cq_e = 0
            cq_w += crw_r
            _pw = cr_w[i, j + 1].try_put(cq_w)
            if _pw == 1: cq_w = 0
            cq_s += crs_r
            _ps = cr_s[i, j].try_put(cq_s)
            if _ps == 1: cq_s = 0
            cq_n += crn_r
            _pn = cr_n[i + 1, j].try_put(cq_n)
            if _pn == 1: cq_n = 0
            ...
            for t in range(NSTEP):
                ...
                # v18: non-blocking. A missed read only DEFERS the increment (credits are
                # additive and the channel retains the value), so nothing is lost.
                ce, ke = cr_e[i, j + 1].try_get()
                if ke == 1: rcred[0] += ce
                cw, kw2 = cr_w[i, j].try_get()
                if kw2 == 1: rcred[1] += cw
                cs, ks2 = cr_s[i + 1, j].try_get()
                if ks2 == 1: rcred[2] += cs
                cn, kn2 = cr_n[i, j].try_get()
                if kn2 == 1: rcred[3] += cn
