...
import allo
from allo.ir.types import float16, int16, int32, UInt, AlloType, Stream, float32, Channel, valid_ready
...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        sys_e: Channel[SYS_W, valid_ready][M, N + 1]
        sys_w: Channel[SYS_W, valid_ready][M, N + 1]
        sys_s: Channel[SYS_W, valid_ready][M + 1, N]
        sys_n: Channel[SYS_W, valid_ready][M + 1, N]
        rtr_e: Channel[Pkt, valid_ready][M, N + 1]
        rtr_w: Channel[Pkt, valid_ready][M, N + 1]
        rtr_s: Channel[Pkt, valid_ready][M + 1, N]
        rtr_n: Channel[Pkt, valid_ready][M + 1, N]
        ...
        def node(pcfg: int32[M, N]):
            ...
            # v17: no router credits. op_v[o]=1 means egress port o still holds an
            # UNGRANTED packet (rq/gt); op_s[o] is the input buffer it came from
            # (-1 = core injection), so the pop can be deferred until the grant.
            op_v: int32[4] = 0; op_s: int32[4] = -1
            ...
            for _pt in range(pcfg[i, j] - 1):
                _r = rtr_e[i, j + 1].try_put(zpkt)   # v17: uniform PushNB
                _r = rtr_w[i, j].try_put(zpkt)
                _r = rtr_s[i + 1, j].try_put(zpkt)
                _r = rtr_n[i, j].try_put(zpkt)
                _p = sys_e[i, j + 1].try_put(zsys)   # v16: uniform PushNB (CIN-150)
                _p = sys_w[i, j].try_put(zsys)   # v16: uniform PushNB (CIN-150)
                _p = sys_s[i + 1, j].try_put(zsys)   # v16: uniform PushNB (CIN-150)
                _p = sys_n[i, j].try_put(zsys)   # v16: uniform PushNB (CIN-150)
            ...
            _r = rtr_e[i, j + 1].try_put(oe_r)
            _r = rtr_w[i, j].try_put(ow_r)
            _r = rtr_s[i + 1, j].try_put(os_r)
            _r = rtr_n[i, j].try_put(on_r)
            _q = sys_e[i, j + 1].try_put(txe_r)   # v16: uniform PushNB (CIN-150)
            _q = sys_w[i, j].try_put(txw_r)   # v16: uniform PushNB (CIN-150)
            _q = sys_s[i + 1, j].try_put(txs_r)   # v16: uniform PushNB (CIN-150)
            _q = sys_n[i, j].try_put(txn_r)   # v16: uniform PushNB (CIN-150)

            for t in range(NSTEP):
                # v17: router RX moved to the loop BOTTOM (after the drain)
                ...
                # Router egress
                # v17 ROUTER EGRESS -- no credits. A port whose packet was NOT
                # granted last cycle RETAINS it (op_v=1) and is skipped here; a free
                # port loads a bubble by default, so always-fire is preserved.
                pop: int32[4] = 0; inj_done: int32 = 0
                idir: int32 = -1
                if csd_pkt[RQ_OFF] == 1: idir = 3 - csd_dir
                if op_v[0] == 0:
                    oe_r = 0
                    if idir == 0: oe_r = csd_pkt; inj_done = 1; op_v[0] = 1; op_s[0] = -1
                    elif hvld[0] == 1 and hit[0] == 0: oe_r = hd[0]; op_v[0] = 1; op_s[0] = 0
                if op_v[1] == 0:
                    ow_r = 0
                    if idir == 1: ow_r = csd_pkt; inj_done = 1; op_v[1] = 1; op_s[1] = -1
                    elif hvld[1] == 1 and hit[1] == 0: ow_r = hd[1]; op_v[1] = 1; op_s[1] = 1
                if op_v[2] == 0:
                    os_r = 0
                    if idir == 2: os_r = csd_pkt; inj_done = 1; op_v[2] = 1; op_s[2] = -1
                    elif hvld[2] == 1 and hit[2] == 0: os_r = hd[2]; op_v[2] = 1; op_s[2] = 2
                if op_v[3] == 0:
                    on_r = 0
                    if idir == 3: on_r = csd_pkt; inj_done = 1; op_v[3] = 1; op_s[3] = -1
                    elif hvld[3] == 1 and hit[3] == 0: on_r = hd[3]; op_v[3] = 1; op_s[3] = 3
                if crv_in >= 0: pop[crv_in] = 1   # local delivery: no backpressure
                
                # v17: a FORWARDED packet is popped only when its egress put is
                # granted (see the END-PUT below). Only the local delivery pops here.
                for d in range(4):
                    if pop[d] == 1:
                ...
                # v13: receive AFTER the drain (gate sees post-drain occupancy)
                rx_w: SYS_W = 0
                rx_e: SYS_W = 0
                rx_n: SYS_W = 0
                rx_s: SYS_W = 0
                if hold_cnt[2] < 2:
                    gw, kw = sys_e[i, j].try_get()
                    if kw == 1: rx_w = gw
                if hold_cnt[3] < 2:
                    ge, ke = sys_w[i, j + 1].try_get()
                    if ke == 1: rx_e = ge
                if hold_cnt[0] < 2:
                    gn, kn = sys_s[i, j].try_get()
                    if kn == 1: rx_n = gn
                if hold_cnt[1] < 2:
                    gs, ks = sys_n[i + 1, j].try_get()
                    if ks == 1: rx_s = gs
                ...
                # v3.1: systolic credit GUARD REMOVED (Channel valid_ready backpressure)
                ...
                if txp_v[0] == 1:
                    ...
                    txn_r = twn   # v13: cleared on grant, not at form
                if txp_v[1] == 1:
                    tws: SYS_W = 0
                    tws[0] = 1
                    tws[1 : 1 + Ty.bits] = txp_d[1].bitcast()
                    txs_r = tws   # v13: cleared on grant, not at form
                if txp_v[2] == 1:
                    tww: SYS_W = 0
                    tww[0] = 1
                    tww[1 : 1 + Ty.bits] = txp_d[2].bitcast()
                    txw_r = tww   # v13: cleared on grant, not at form
                if txp_v[3] == 1:
                    twe: SYS_W = 0
                    twe[0] = 1
                    twe[1 : 1 + Ty.bits] = txp_d[3].bitcast()
                    txe_r = twe   # v13: cleared on grant, not at form
                if crv_vld == 1:
                    if crv_mode == 1:
                ...
                # v17 ROUTER END-PUT -- rq/gt. On grant the port frees and, if the
                # packet was FORWARDED (op_s>=0), its source buffer pops now. An
                # ungranted packet stays in oe_r AND in rbuf, so nothing is lost.
                ge = rtr_e[i, j + 1].try_put(oe_r)
                if ge == 1 and op_v[0] == 1:
                    op_v[0] = 0
                    if op_s[0] >= 0:
                        for sfe in range(BUF_DEPTH - 1):
                            rbuf[0, sfe] = rbuf[0, sfe + 1]
                        rbcnt[0] -= 1
                gw2 = rtr_w[i, j].try_put(ow_r)
                if gw2 == 1 and op_v[1] == 1:
                    op_v[1] = 0
                    if op_s[1] >= 0:
                        for sfw in range(BUF_DEPTH - 1):
                            rbuf[1, sfw] = rbuf[1, sfw + 1]
                        rbcnt[1] -= 1
                gs2 = rtr_s[i + 1, j].try_put(os_r)
                if gs2 == 1 and op_v[2] == 1:
                    op_v[2] = 0
                    if op_s[2] >= 0:
                        for sfs in range(BUF_DEPTH - 1):
                            rbuf[2, sfs] = rbuf[2, sfs + 1]
                        rbcnt[2] -= 1
                gn2 = rtr_n[i, j].try_put(on_r)
                if gn2 == 1 and op_v[3] == 1:
                    op_v[3] = 0
                    if op_s[3] >= 0:
                        for sfn in range(BUF_DEPTH - 1):
                            rbuf[3, sfn] = rbuf[3, sfn + 1]
                        rbcnt[3] -= 1
                oe = sys_e[i, j + 1].try_put(txe_r)     # always fire (bubble or real)
                if oe == 1 and txe_r[0] == 1: txp_v[3] = 0
                ow = sys_w[i, j].try_put(txw_r)
                if ow == 1 and txw_r[0] == 1: txp_v[2] = 0
                os_ = sys_s[i + 1, j].try_put(txs_r)
                if os_ == 1 and txs_r[0] == 1: txp_v[1] = 0
                on_ = sys_n[i, j].try_put(txn_r)
                if on_ == 1 and txn_r[0] == 1: txp_v[0] = 0

                # v17 ROUTER RECEIVE -- placed AFTER the drain so the gate sees
                # post-drain occupancy (the v11 lesson), and gated on room so a
                # packet is never accepted and then silently dropped (the v3.2 bug).
                p_w: Pkt = 0
                p_e: Pkt = 0
                p_n: Pkt = 0
                p_s: Pkt = 0
                if rbcnt[0] < BUF_DEPTH:
                    qw, mw = rtr_e[i, j].try_get()
                    if mw == 1: p_w = qw
                if rbcnt[1] < BUF_DEPTH:
                    qe, me = rtr_w[i, j + 1].try_get()
                    if me == 1: p_e = qe
                if rbcnt[2] < BUF_DEPTH:
                    qn, mn = rtr_s[i, j].try_get()
                    if mn == 1: p_n = qn
                if rbcnt[3] < BUF_DEPTH:
                    qs, ms = rtr_n[i + 1, j].try_get()
                    if ms == 1: p_s = qs
                fin: Pkt[4] = 0
        ...
        def drv_w(din_w: Ty[M, LANELEN], vd_w: int32[M, LANELEN], pcfg: int32[M, N]):
            ...
            for _pt in range(pcfg[0, 0] - 1):
                with allo.meta_for(0, M) as r:
                    _d = sys_e[r, 0].try_put(zw)   # v16: uniform PushNB (CIN-150)
            for t in range(NSTEP):
                with allo.meta_for(0, M) as r:
                    w: SYS_W = 0
                    ...
                    # v15: RETAIN UNTIL GRANTED. Non-blocking put, and the
                    # pointer advances only on grant -- so a refused word is
                    # re-offered next cycle instead of being skipped, and a
                    # stalled consumer can never block this driver.
                    og = sys_e[r, 0].try_put(w)
                    if og == 1 and w[0] == 1: sp[r] += 1
        ...
        def drv_e(din_e: Ty[M, LANELEN], vd_e: int32[M, LANELEN], pcfg: int32[M, N]):
            ...
            for _pt in range(pcfg[0, 0] - 1):
                with allo.meta_for(0, M) as r:
                    _d = sys_w[r, N].try_put(zw)   # v16: uniform PushNB (CIN-150)
            for t in range(NSTEP):
                with allo.meta_for(0, M) as r:
                    w: SYS_W = 0
                    if sp[r] < LANELEN and t >= sp[r]:
        ...
        def drv_n(din_n: Ty[N, LANELEN], vd_n: int32[N, LANELEN], pcfg: int32[M, N]):
            ...
            for _pt in range(pcfg[0, 0] - 1):
                with allo.meta_for(0, N) as c:
                    _d = sys_s[0, c].try_put(zw)   # v16: uniform PushNB (CIN-150)
            for t in range(NSTEP):
                with allo.meta_for(0, N) as c:
                    w: SYS_W = 0
                    ...
                    # re-offered next cycle instead of being skipped, and a
                    # stalled consumer can never block this driver.
                    og = sys_s[0, c].try_put(w)
                    if og == 1 and w[0] == 1: sp[c] += 1
        ...
        def drv_s(din_s: Ty[N, LANELEN], vd_s: int32[N, LANELEN], pcfg: int32[M, N]):
            ...
            for _pt in range(pcfg[0, 0] - 1):
                with allo.meta_for(0, N) as c:
                    _d = sys_n[M, c].try_put(zw)   # v16: uniform PushNB (CIN-150)
            for t in range(NSTEP):
                with allo.meta_for(0, N) as c:
                    w: SYS_W = 0
                    if sp[c] < LANELEN and t >= sp[c]:
        ...
        def rdrv_w(rdin_w: int32[M, LANELEN], pcfg: int32[M, N]):
            ...
            for t in range(NSTEP):
                with allo.meta_for(0, M) as r:
                    ...
                    # v17: RETAIN UNTIL GRANTED at the router injection edge -- the v15 fix.
                    og = rtr_e[r, 0].try_put(pw)
                    if og == 1 and pw[RQ_OFF] == 1: sp[r] += 1
        ...
        def rdrv_n(rdin_n: int32[N, LANELEN], pcfg: int32[M, N]):
            ...
            for t in range(NSTEP):
                with allo.meta_for(0, N) as c:
                    ...
                    # v17: RETAIN UNTIL GRANTED at the router injection edge -- the v15 fix.
                    og = rtr_s[0, c].try_put(pw)
                    if og == 1 and pw[RQ_OFF] == 1: sp[c] += 1
...
def get_scheduled_eva(Ty: AlloType = float16, pipeline_node=False, partition_rf=False, extended=False):
    ...
    if partition_rf:
        for i in range(M):
            for j in range(N):
                for buf in ("irf", "drf", "drf_full", "rbuf", "rbcnt", "op_v", "op_s", "hold_v", "hold_cnt",#,
...
if __name__ == "__main__":
    ...
    s.build(target="systemc", mode="csyn", project=P)
