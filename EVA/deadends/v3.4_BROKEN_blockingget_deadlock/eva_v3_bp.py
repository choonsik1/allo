    ...
    def top(
        ...
        def node(pcfg: int32[M, N]):
            ...
            for t in range(NSTEP):
                ...
                # v3.4 BACKPRESSURE: only accept a word when this direction's
                # 2-deep hold has room. Previously the get() was UNCONDITIONAL and a
                # full hold silently DROPPED the word -- the credit plane existed to
                # make that drop unreachable. Gating the get() instead makes the
                # Channel[.., valid_ready] handshake itself stall the sender, which is
                # what the golden EVA rq/gt does. hold idx: 0<-rx_n 1<-rx_s 2<-rx_w 3<-rx_e
                rx_w: SYS_W = 0
                rx_e: SYS_W = 0
                rx_n: SYS_W = 0
                rx_s: SYS_W = 0
                if hold_cnt[2] < 2: rx_w = sys_e[i, j].get()
                if hold_cnt[3] < 2: rx_e = sys_w[i, j + 1].get()
                if hold_cnt[0] < 2: rx_n = sys_s[i, j].get()
                if hold_cnt[1] < 2: rx_s = sys_n[i + 1, j].get()
