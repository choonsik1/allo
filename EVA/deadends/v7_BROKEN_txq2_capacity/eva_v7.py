...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        def node(pcfg: int32[M, N]):
            ...
            # v7: 2-SLOT TX sync_register per direction, held in REGISTERS (not a FIFO
            # channel) -- the same slack golden EVA's sync_register gives, and the mirror
            # of hold_v[4,2] on the receive side. txp_c = occupancy 0..2. With zero-buffer
            # Channel links there was NO slack anywhere, which closed v6's deadlock ring.
            txp_d: Ty[4, 2] = 0
            txp_c: int32[4] = 0
            ...
            for t in range(NSTEP):
                ...
                # v6 RX: grant only when this direction's 2-deep hold has room.
                # try_get (not a gated blocking get -- that freezes the thread).
                rx_w: SYS_W = 0
                rx_e: SYS_W = 0
                rx_n: SYS_W = 0
                rx_s: SYS_W = 0
                ...
                if sb_v[0] == 1 and sb_rtr[0] == 0 and sb_dst[0] >= 12:
                    if sb_rvld[0] == 1 and txp_c[sb_dst[0] & 3] >= 2: retire_ok = 0
                if sb_v[0] == 1 and retire_ok == 1:
                    ...
                    if sb_rtr[0] == 1:
                        ...
                        if sb_rvld[0] == 1:       # bubble result = no-op
                            _td: int32 = sb_dst[0] & 3
                            txp_d[_td, txp_c[_td]] = wb
                            txp_c[_td] += 1
                            txp_r[sb_dst[0] & 3] = 1
                    else:
                        if sb_rvld[0] == 1:
                ...
                if txp_c[0] > 0:
                    twn: SYS_W = 0
                    twn[0] = 1
                    twn[1 : 1 + Ty.bits] = txp_d[0, 0].bitcast()
                    txn_r = twn   # v7: head of the 2-slot TX queue; popped only on grant
                if txp_c[1] > 0:
                    tws: SYS_W = 0
                    tws[0] = 1
                    tws[1 : 1 + Ty.bits] = txp_d[1, 0].bitcast()
                    txs_r = tws   # v7: head of the 2-slot TX queue; popped only on grant
                if txp_c[2] > 0:
                    tww: SYS_W = 0
                    tww[0] = 1
                    tww[1 : 1 + Ty.bits] = txp_d[2, 0].bitcast()
                    txw_r = tww   # v7: head of the 2-slot TX queue; popped only on grant
                if txp_c[3] > 0:
                    twe: SYS_W = 0
                    twe[0] = 1
                    twe[1 : 1 + Ty.bits] = txp_d[3, 0].bitcast()
                    txe_r = twe   # v7: head of the 2-slot TX queue; popped only on grant
                if crv_vld == 1:
                    if crv_mode == 1:
                        ...
                        _tc: int32 = crv_addr & 3
                        if txp_c[_tc] < 2:
                            txp_d[_tc, txp_c[_tc]] = crv_data
                            txp_c[_tc] += 1
                        txp_r[crv_addr & 3] = 1
                ...
                rtr_s[i + 1, j].put(os_r)
                rtr_n[i, j].put(on_r)
                # v6 RQ/GT: try_put == assert rq; success == gt. Ungranted words stay
                # pending and retry next cycle (stall-not-drop). Only real words go out.
                if txe_r[0] == 1:
                    oe = sys_e[i, j + 1].try_put(txe_r)
                    if oe == 1:
                        txp_d[3, 0] = txp_d[3, 1]
                        txp_c[3] -= 1
                if txw_r[0] == 1:
                ...
                with allo.meta_for(0, M) as r:
                    ...
                    if sp[r] < LANELEN and t >= sp[r]:
                        if vd_w[r, sp[r]] == 0:
                            ...
                            sp[r] += 1
                    if w[0] == 1:
                        od = sys_e[r, 0].try_put(w)
                        if od == 0: sp[r] -= 1   # not granted -> resend this slot
                ...
                with allo.meta_for(0, N) as c:
                    ...
                    if sp[c] < LANELEN and t >= sp[c]:
                        if vd_n[c, sp[c]] == 0:
                            ...
                            sp[c] += 1
                    if w[0] == 1:
                        od = sys_s[0, c].try_put(w)
                        if od == 0: sp[c] -= 1   # not granted -> resend this slot
