...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        # DEBUG word-flow counters (16b packed, see v14 header)
        #   dbg_a = words RECEIVED (low16) | operand CONSUMES (high16)
        #   dbg_b = GRANTS         (low16) | real words TRANSMITTED (high16)
        dbg_a: int32[M, N], dbg_b: int32[M, N],
        ...
        @df.kernel(mapping=[M, N], args=[prime_cfg, dbg_a, dbg_b])
        def node(pcfg: int32[M, N], da: int32[M, N], db: int32[M, N]):
            i, j = df.get_pid()
            ...
            n_rx: int32 = 0; n_cn: int32 = 0    # v14 counters
            n_gr: int32 = 0; n_tx: int32 = 0
            ...
            for t in range(NSTEP):
                ...
                if c1 >= 0:
                    hold_v[c1, 0] = hold_v[c1, 1]; hold_cnt[c1] -= 1; n_cn += 1
                if c2 >= 0 and c2 != c1:
                    hold_v[c2, 0] = hold_v[c2, 1]; hold_cnt[c2] -= 1; n_cn += 1

                da[i, j] = (n_rx & 65535) | ((n_cn & 65535) << 16)
                db[i, j] = (n_gr & 65535) | ((n_tx & 65535) << 16)
                ...
                for d in range(4):
                    if rxvld[d] == 1 and hold_cnt[d] < 2:
                        ...
                        if d == 2: n_rx += 1        # v14b: WEST input only
                ...
                if oe == 1 and txe_r[0] == 1: txp_v[3] = 0; n_tx += 1
...
def get_eva_top_extended(Ty: AlloType = float16):
    ...
    def top_extended(
        ...
        @df.kernel(mapping=[M, N], args=[dbg_a, dbg_b])
        def node(da: int32[M, N], db: int32[M, N]):
            i, j = df.get_pid()
            irf: int32[IRF_DEPTH] = 0
            drf: Ty[DRF_DEPTH] = 0
