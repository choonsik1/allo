...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        prime_cfg: int32[M, N],
        # DEBUG: dbg_t = last iteration reached; dbg_s = packed flow-control snapshot
        #   bits 0..7  = hold_cnt[0..3]  (2 bits each)
        #   bits 8..11 = txp_v[0..3]
        dbg_t: int32[M, N], dbg_s: int32[M, N],
    ):
        ...
        @df.kernel(mapping=[M, N], args=[prime_cfg, dbg_t, dbg_s])
        def node(pcfg: int32[M, N], dt: int32[M, N], ds: int32[M, N]):
            i, j = df.get_pid()
            ...
            for t in range(NSTEP):
                dt[i, j] = t
                ds[i, j] = (hold_cnt[0] & 3) | ((hold_cnt[1] & 3) << 2) \
                         | ((hold_cnt[2] & 3) << 4) | ((hold_cnt[3] & 3) << 6) \
                         | ((txp_v[0] & 1) << 8) | ((txp_v[1] & 1) << 9) \
                         | ((txp_v[2] & 1) << 10) | ((txp_v[3] & 1) << 11)
                ...
                # v11 ORDERING FIX: receive AFTER the drain. The gate used to read
                # hold_cnt BEFORE the slot was freed, so every node was pessimistic by
                # one slot each cycle (a 2-deep hold behaved as 1). The RTL is
                # combinational -- wr_gt_o = ~oflw_data_vld reflects THIS cycle's drain,
                # i.e. it accepts BECAUSE it is simultaneously draining.
                rx_w: SYS_W = 0
                rx_e: SYS_W = 0
                rx_n: SYS_W = 0
                rx_s: SYS_W = 0
                ...
                if txp_v[0] == 1:
                    ...
                    txn_r = twn   # v9: cleared on grant, not at form
                if txp_v[1] == 1:
                    tws: SYS_W = 0
                    tws[0] = 1
                    tws[1 : 1 + Ty.bits] = txp_d[1].bitcast()
                    txs_r = tws   # v9: cleared on grant, not at form
                if txp_v[2] == 1:
                    tww: SYS_W = 0
                    tww[0] = 1
                    tww[1 : 1 + Ty.bits] = txp_d[2].bitcast()
                    txw_r = tww   # v9: cleared on grant, not at form
                if txp_v[3] == 1:
                    twe: SYS_W = 0
                    twe[0] = 1
                    twe[1 : 1 + Ty.bits] = txp_d[3].bitcast()
                    txe_r = twe   # v9: cleared on grant, not at form
                if crv_vld == 1:
                    if crv_mode == 1:
