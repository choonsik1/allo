...
SC_MODULE(drim4hls) {
    ...
    // writeback's MMIO ports promoted to the top. Together with the four
    // memory ports these are the only dataflow channels drim4hls exposes; the
    // six inter-stage Combinationals below stay private, so parse_sc_module
    // sees a 6-port IP rather than the whole pipeline.
    ...
    wb("Writeback") {
        ...
        // Allo: forward writeback's MMIO channels to drim4hls's own ports.
        wb.mmio_in(mmio_in);
        wb.mmio_out(mmio_out);
        wb.mmio_out_b(mmio_out_b);
    }
