...
class Top: public sc_module {
    ...
    // Allo: the MMIO channels must terminate somewhere or elaboration fails
    // with E109 "port not bound". mmio_out also needs a CONSUMER -- a Push()
    // with nobody reading blocks forever -- so the monitor below drains it and
    // reports what the program stored. mmio_in is left with no producer, so a
    // program that loads from >= MMIO_BASE blocks on Pop(), which is the
    // correct behaviour for a starved channel.
...
int sc_main(int argc, char * argv[]) {
    ...
    // Allo: upstream hardcodes a path into the original author's home directory
    // here, with the argv handling commented out -- so the loader silently reads
    // nothing, imem stays all zero, and the CPU fetches zeros until it walks off
    // the end. Take the program from the command line instead.
    if (argc < 2) {
        std::cerr << "usage: " << argv[0] << " <program.mem>" << std::endl;
        return -1;
    }
