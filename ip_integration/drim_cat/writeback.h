...
SC_MODULE(writeback) {
    ...
    // A load/store whose WORD address is >= MMIO_BASE is a channel access, not
    // a dmem access, so the processor can emit EVA router packets with an
    // ordinary `sw` and stall on an ordinary `lw`. DCACHE_SIZE words of real
    // memory sit below it, so the two can never alias. Same hook as the
    // vhls-riscv IP and the HL5 port: it is what gives a memory-mapped
    // processor a streaming interface without touching decode or the ALU.
    // Two output channels, selected by the STORE ADDRESS: MMIO_BASE goes to
    // lane 0 and MMIO_BASE+1 to lane 1. Same trick as the Vitis IP's
    // MMIO_OUT26 / MMIO_OUT26B, and it is what lets ONE processor drive a 2xN
    // grid where each node needs its own program image.
    static const unsigned MMIO_BASE = DCACHE_SIZE;
    Connections::In  < sc_uint<XLEN> > CCS_INIT_S1(mmio_in);
    Connections::Out < sc_uint<XLEN> > CCS_INIT_S1(mmio_out);
    Connections::Out < sc_uint<XLEN> > CCS_INIT_S1(mmio_out_b);
    ...
    void writeback_th(void) {
        WRITEBACK_RST: {
            ...
            dout.Reset();
            dmem_in.Reset();
            // Allo: every Connections port must be Reset() in its thread's
            // reset block, or the channel warns CONNECTIONS-101 and its
            // val/rdy come out of reset undriven. Adding ports without this
            // is the easy mistake -- it cost a debug cycle on HL5.
            mmio_in.Reset();
            mmio_out.Reset();
            mmio_out_b.Reset();
        ...
        WRITEBACK_BODY: while (true) {
            ...
            // Placed BEFORE the normal path so the dmem branches below are
            // untouched. The load arm feeds mem_dout exactly as LW_LOAD does,
            // so writeback needs no further change; the store arm emits the
            // word and issues no dmem transaction at all.
            if (aligned_address >= MMIO_BASE) {
                if (input.ld != NO_LOAD)
                    mem_dout = mmio_in.Pop();          // blocking, like a FIFO
                else if (input.st != NO_STORE) {
                    if (aligned_address == MMIO_BASE)
                        mmio_out.Push((sc_uint<XLEN>) input.mem_datain.to_uint());
                    else
                        mmio_out_b.Push((sc_uint<XLEN>) input.mem_datain.to_uint());
                }
            }
			else if (input.ld != NO_LOAD) { // a load is requested
