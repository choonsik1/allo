...
# Set to an allo.IPModule to have a hand-written HLS processor drive the west
# router port in place of the rdrv_w kernel -- see rv_stream_ip.cpp. The IP
# carries the PE's boot sequence in its own ROM, so the processor is what
# programs the PE. Leave None for the stock chip. Only meaningful at M == N == 1;
# the IP's baked-in NSTEP/PRIME_TOKENS must match this module's.
RV_PROGRAMMER = None
...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        # The west router driver is a hand-written HLS soft processor rather than
        # an Allo kernel: it injects the packet sequence that loads the PE's
        # instruction register file and operands and then sets fetch_en, so the PE
        # executes code the processor put there. RV_PROGRAMMER must be set before
        # calling this builder. The stock packet-replaying rdrv_w kernel is in the
        # original chip at Vitis_HLS/bubble_model/.../prime/.
        #
        # It cannot be selected with a Python `if` in here: Allo traces this whole
        # function body, so a conditional around a kernel definition is compiled as
        # IR rather than evaluated ("Unsupported global variable").
        # args=[rin_w, prime_cfg] is kept even though the processor reads
        # neither: the top-level argument order follows kernel-appearance order,
        # so dropping them moves rin_e/n/s after the out_* arguments and the
        # vitis_hls backend rejects it ("Output arguments must appear at the
        # end"). The host array simply goes unread now.
        @df.kernel(mapping=[1], args=[rin_w, prime_cfg])
        def rdrv_w(rdin_w: int32[M, LANELEN], pcfg: int32[M, N]):
            RV_PROGRAMMER(cr_e[0, 0], rtr_e[0, 0])
