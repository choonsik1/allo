# EVA with the PE program supplied by an HLS soft processor over a stream.
#
# Sibling of eva_fp16_skid.py. That variant has the processor *replace* the west
# router driver, so the processor has to implement EVA's credit protocol itself.
# This one is the EVA-faithful arrangement: the processor only produces the
# program image, and EVA's own rdrv_w kernel buffers it into a local array and
# does the credit-gated replay -- exactly the split between a host and the array.
#
# Set RV_PKTSRC to an allo.IPModule (see rv_stream_ip.cpp, top rv_eva_pktsrc)
# before calling the builder. NPKT must equal the packet count baked into the
# IP's ROM: both the producer and the consumer loop on it as a constant, so a
# mismatch deadlocks rather than fails.
...
NPKT = 24          # packets the processor hands each west lane
RV_PKTSRC = None   # allo.IPModule for the program source (see header)
...
def get_eva_top(Ty: AlloType = float16):
    ...
    def top(
        ...
        # program image, processor -> west router driver (one per lane)
        pkt_s: Stream[Pkt, STREAM_DEPTH][M, 1]
        ...
        # The soft processor: one instance per west lane, each emitting its
        # lane's program image out of its own ROM. It reads nothing -- no credit
        # protocol, no timing -- so it is a pure producer, which is the whole
        # point of this variant.
        @df.kernel(mapping=[1])
        def rvprog():
            # ONE processor drives every west lane, not one per lane. meta_for
            # would instantiate the same IP top M times, and M instances all
            # carry the same ROM -- fine while every PE runs the same program,
            # useless once each node needs its own stationary weight. So the IP
            # takes one output stream per lane and the address picks the lane.
            # meta_if is a build-time gate, so only the matching arity is traced.
            with allo.meta_if(M == 1):
                RV_PKTSRC(pkt_s[0, 0])
            with allo.meta_else():
                RV_PKTSRC(pkt_s[0, 0], pkt_s[1, 0])

        # EVA's own west router driver, unchanged except for where the packets
        # come from: it drains the processor's image into a local array (the
        # "memory") and then replays it with the stock credit-gated loop.
        # args=[rin_w, prime_cfg] is kept although rin_w is now unread -- the
        # top-level argument order follows kernel-appearance order, so dropping
        # it moves rin_e/n/s after the out_* arguments.
        ...
        def rdrv_w(rdin_w: int32[M, LANELEN], pcfg: int32[M, N]):
            ...
            # Fill BEFORE priming. The array's other kernels simply block on
            # rtr_e until the replay loop starts, and the processor has no
            # inputs, so this cannot deadlock as long as NPKT matches its ROM.
            buf: Pkt[M, NPKT] = 0
            with allo.meta_for(0, M) as r:
                for k in range(NPKT):
                    buf[r, k] = pkt_s[r, 0].get()
            for _pt in range(pcfg[0, 0] - 1):
                with allo.meta_for(0, M) as r:
                    rtr_e[r, 0].put(zp)
            for t in range(NSTEP):
